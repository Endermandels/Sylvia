extends GutTest
var main_node = load("res://start_menu/start_menu.tscn").instantiate()
...
func test_battle_menu_default():
	battle_scene = load("res://battle/BattleScene.tscn").instantiate()
	get_tree().get_root().add_child(battle_scene)
	var control_nodes = get_control_nodes(battle_scene)
	for node in control_nodes:
		assert_true(node.get_theme_default_font_size() == 20)
#acceptance test
...
func test_no_menu_dyslexia_and_no_large_text():
	var control_nodes = get_control_nodes(main_node)
	for node in control_nodes:
		if node is Button:
			var font = load("res://ui_theme/default.tres")
